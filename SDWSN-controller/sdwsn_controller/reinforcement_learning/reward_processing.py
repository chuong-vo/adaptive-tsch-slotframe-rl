...
class EmulatedRewardProcessing(RewardProcessing):
    def __init__(
        ...
        **kwargs,
        ...
        self.__reliability_min = config.performance_metrics.pdr.min
        self.__reliability_max = config.performance_metrics.pdr.max
    ...
    def calculate_reward(self, alpha, beta, delta, _) -> dict:
        ...
        pdr_wam, pdf_mean, pdr_normalized = self.__get_network_pdr()
        # Calculate the reward
        reward = 2 - 1 * (
            alpha * power_normalized
            + beta * delay_normalized
            - delta * pdr_normalized
        )
        info = {
    ...
    def __normalize_metric(self, value, min_value, max_value, offset, metric_name):
        if value is None:
            return offset
        if min_value is None or max_value is None:
            logger.warning(
                "%s bounds missing for normalization; using offset only", metric_name
            )
            return offset
        span = max_value - min_value
        if span <= 0:
            logger.warning(
                "%s bounds invalid (min=%s, max=%s); using offset only",
                metric_name,
                min_value,
                max_value,
            )
            return offset
        normalized = offset + ((value - min_value) / span)
    ...
    def __get_network_power_consumption(self):
        ...
        power_wam, power_mean = self.__power_weighted_arithmetic_mean(
            power_samples)
        if power_wam is None:
            logger.warning("Power samples unavailable; defaulting to normalization offset")
            return 0.0, 0.0, self.__power_norm_offset
        # We now need to normalize the power WAM
        normalized_power = self.__normalize_metric(
            power_wam,
            self.__power_min,
            self.__power_max,
            self.__power_norm_offset,
            "power",
        )
    ...
    def __power_weighted_arithmetic_mean(self, power_samples):
        ...
        if sum_weights == 0:
            logger.warning("Power weights sum to zero; cannot compute WAM")
            return None, None
        ...
        if all_power.size == 0:
            logger.warning("No valid power samples collected")
            return None, None
    ...
    def __get_network_delay(self):
        ...
        delay_wam, delay_mean = self.__delay_weighted_arithmetic_mean(
            delay_samples)
        if delay_wam is None:
            logger.warning("Delay samples unavailable; defaulting to normalization offset")
            return 0.0, 0.0, self.__delay_norm_offset
        # We now need to normalize the power WAM
        normalized_delay = self.__normalize_metric(
            delay_wam,
            self.__delay_min,
            self.__delay_max,
            self.__delay_norm_offset,
            "delay",
        )
    ...
    def __delay_weighted_arithmetic_mean(self, delay_samples):
        ...
        sum_weights = weights_np.sum()
        if sum_weights == 0:
            logger.warning("Delay weights sum to zero; cannot compute WAM")
            return None, None
        ...
        if all_delay.size == 0:
            logger.warning("No valid delay samples collected")
            return None, None
    ...
    def __get_network_pdr(self):
        ...
        pdr_wam, pdr_mean = self.__pdr_weighted_arithmetic_mean(
            pdr_samples)
        if pdr_wam is None:
            logger.warning("PDR samples unavailable; defaulting to normalization offset")
            return 0.0, 0.0, self.__reliability_norm_offset
        logger.debug(f'normalized pdr {pdr_wam}')
        pdr_normalized = self.__normalize_metric(
            pdr_wam,
            self.__reliability_min,
            self.__reliability_max,
            self.__reliability_norm_offset,
            "pdr",
        )
        return pdr_wam, pdr_mean, pdr_normalized
    ...
    def __pdr_weighted_arithmetic_mean(self, pdr_samples):
        ...
        sum_weights = weights_np.sum()
        if sum_weights == 0:
            logger.warning("PDR weights sum to zero; cannot compute WAM")
            return None, None
        normalized_weights = weights_np/sum_weights
        logger.debug(f'normalized weights {normalized_weights}')
        all_pdr = np.array(all_pdr_samples)
        if all_pdr.size == 0:
            logger.warning("No valid PDR samples collected")
            return None, None
