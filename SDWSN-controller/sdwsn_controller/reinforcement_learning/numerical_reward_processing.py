...
class NumericalRewardProcessing(RewardProcessing):
    ...
    def calculate_reward(self, alpha, beta, delta, sf_size):
        ...
        power_normalized = float(np.clip(
            self.power_trendpoly(sf_size) + self.__power_norm_offset,
            0.0,
            1.0,
        ))
        # Calculate delay consumption
        delay_normalized = float(np.clip(
            self.delay_trendpoly(sf_size) + self.__delay_norm_offset,
            0.0,
            1.0,
        ))
        # Calculate pdr consumption
        pdr_normalized = float(np.clip(
            self.pdr_trendpoly(sf_size) + self.__reliability_norm_offset,
            0.0,
            1.0,
        ))
        # Calculate the reward
        reward = float(2-1*(alpha*power_normalized+beta *
