...
class DOCKERConfig:
    ...
    def from_json(cls, json_object=None):
        ...
        image = json_object.get(IMAGE, DEFAULT_IMAGE)
        script_folder = json_object.get(SCRIPT_FOLDER, DEFAULT_SCRIPT_FOLDER)
        target = _expand(json_object.get(TARGET, DEFAULT_TARGET))
        contiki = _expand(json_object.get(CONTIKI))
        simulation_script = json_object.get(SIMULATION_SCRIPT, DEFAULT_SIM_SCRIPT)
        privileged = json_object.get(PRIVILEGED, DEFAULT_PRIVILEGED)
        detach = json_object.get(DETACH, DEFAULT_DETACH)
        port = json_object.get(PORT, DEFAULT_PORT)

        # Fallbacks for contiki path in container mounts
        if not contiki or (isinstance(contiki, str) and len(contiki.strip()) == 0):
            contiki = os.environ.get('CONTIKI_NG') or os.environ.get('CNG_PATH')
