...
class Network:
    def __init__(self, config: Any, socket: Any) -> None:
    ...
    def routes_sendall(self):
        ...
        if payload:
            num_pkts += 1
            logger.debug(f"Sending routing packet {num_pkts} with {len(payload)} bytes")
    ...
    def tsch_print(self):
        ...
        df = pd.DataFrame(index=range(max_rows + 1), columns=range(max_columns + 1))
        ...
        if self.processing_window is not None:
            ...
            with Progress(transient=True) as progress:
                ...
                while True:
                    ...
                    progress.update(
                        task, completed=min(current_sequence, self.processing_window)
                    )
    ...
    def start_socket(self) -> bool:
        if self.socket is not None:
            ...
            # Read serial
            self.read_socket_thread = threading.Thread(target=self.read_socket)
            self.read_socket_thread.start()
            return True

    def stop_socket(self):
        if self.socket is not None:
            logger.debug(f"Shutting down socket {self.network_running}")
